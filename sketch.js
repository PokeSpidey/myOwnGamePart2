...
function draw() {
  ...
  if (gameState == 0) {
    ...
    text("2. Total Lives: 5, A Life Is Lost When An Imperial Troop Touches The Planet Tatooine.", width/2-250, 150);
  ...
  if (gameState == 1) {
    milleniumFalcon.x = width-280;
    tatooine.x = width+310;
    ...
    milleniumFalcon.destroy();
    mf2.destroy();
    text("GAME OVER, REFRESH TO RETRY!", width/2-350, height/2);
  }
    ...
    milleniumFalcon.destroy();
    mf2.destroy();
    text("CONGRATULATIONS! HOPE YOU ENJOYED!", width/2-350, height/2);
  }
}
